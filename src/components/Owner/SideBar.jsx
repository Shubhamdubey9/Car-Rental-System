...
const SideBar = ({ closeSidebar }) => {
  ...
  const handleProfileChange = async (e) => {
    try {
      ...
      const response = await axios.post(
        `${OWNER_API_END_POINT}/update-image`,
        formData,
        {
  ...
  return (
    <div className="h-full w-full md:w-64 bg-white border-r border-gray-100 flex flex-col py-22 md:py-15 lg:py-12 px-4 md:px-6 transition-all duration-300">
      {/* Profile Section */}
      <div className="flex flex-col items-center mb-8">
        ...
        <span className="mt-4 font-semibold text-blue-700 text-base text-center">
          {user?.name}
        </span>
        <span className="text-xs text-gray-500 text-center">{user?.email}</span>
      </div>

      {/* Title */}
      <h2 className="text-lg md:text-xl font-bold text-blue-700 text-center mb-4 -mt-4 ">
        Owner Panel
      </h2>
      ...
      <nav className="flex flex-col gap-2">
        {ownerMenuLinks.map((item) => (
          <NavLink
            ...
            className={({ isActive }) =>
              `flex items-center gap-3 py-2 px-3 rounded-md font-medium text-sm md:text-base transition-all duration-150 ${
                isActive
